...
var DEFAULT_APP_TOKEN = 'APP_TOKEN';
var RESTART_DELAY_MS = 1000;

/**
 * The global the natives call with a launch or resume URL: on iOS Cordova's own
 * `CDVHandleOpenURL` invokes it once the page has loaded, and on Android
 * `cordova-plugin-customurlscheme` calls it from the launch intent.
 *
 * It has to be defined while this script is parsed, not from `deviceready`:
 * the call arrives as soon as the page is ready, and without this global the
 * URL that launched the app is dropped before any of the app's code sees it.
 */
window.handleOpenURL = function (url) {
    if (!url) return;

    var openUrl = String(url);
    // Android hands the URL over encoded.
    if (openUrl.indexOf('://') === -1 && openUrl.indexOf('%') !== -1) {
        try {
            openUrl = decodeURIComponent(openUrl);
        } catch (error) {
            logOutput('Could not decode the incoming URL: ' + error.message);
        }
    }

    processDeepLink(openUrl);
};
...
function onDeviceReady() {
    ...
    // Subscribe before SDK setup: the deeplinks plugin holds the launch event
    // only until the first subscriber appears, and setup finishes later.
    ...
    plugin.setup(
        ...
        function (result) {
            ...
            if (pendingDeepLinkUrl) {
                var replayUrl = pendingDeepLinkUrl;
                pendingDeepLinkUrl = null;
                logOutput('Processing pending deep link: ' + replayUrl);
                processDeepLink(replayUrl);
            }
        },
        function (error) {
...
function testDeepLink() {
    var testUrl = 'userpilot-APP_TOKEN://sdk/experience_preview/49?type=mobile_content';
